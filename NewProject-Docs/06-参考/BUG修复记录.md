...
> 记录项目开发过程中遇到的BUG及其修复方案，作为历史BUG参考，便于新AI或开发者快速了解历史问题，避免重复踩坑。
...
将 DateCode 改为实际编译日期 `20260724`，并建立版本更新规则，每次发版必须同步更新。
...
> **注**：此修复在 v0.2.3 被 BUG-010 回退（30 秒周期性上报会触发 z2m 无意义 action 事件）。
...
> **注**：此修复在 v1.0.10 被进一步优化（BUG-012 的参数选择对新设备无效，详见 v1.0.10 修复）。
